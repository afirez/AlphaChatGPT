...
class NeuralNetworkViz(QtWidgets.QWidget):
    ...
    def show_network(self, painter: QtGui.QPainter):
        ...
        active_outputs = self.mario.network.out > 0.5
        
        # 绘制节点
        for layer, num_nodes in enumerate(layer_nodes[1:],1):
            activations = self.mario.network.params['A' + str(layer)]
            for node in range(num_nodes):
                x_loc,y_loc = self.neuron_locations[(layer, node)]
                
                # Hidden layers
                if layer < len(layer_nodes) - 1:
                    saturation = max(min(activations[node], 1.0), 0.0)
                ...
                # Output layer
                    ...
                    painter.drawText(x_loc - self.neuron_radius/2, y_loc + 4*self.neuron_radius, text)
                ...
                painter.drawEllipse(x_loc - self.neuron_radius, y_loc, self.neuron_radius*2, self.neuron_radius*2)
